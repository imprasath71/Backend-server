...
app.post('/certificate', (req, res) => {
    ...
    sql.query(`INSERT INTO certification (certname,certid,email) VALUES ('${name}','${id}','${email}')`,(err, result) => {
  if (err) {
    console.log(err);
  } else {
      res.send(result);
...
app.post('/display',(req,res) => {
    ...
    sql.query(`SELECT * FROM certification  WHERE email = '${email}'`,(err,result) => {
        if(err){
            console.log(err);
        }
        else{
...
app.post('/delete/:id',(req,res) => {
    ...
    sql.query(`DELETE FROM certification WHERE certid = '${id}'`, (err,result) => {
        if(err)
        {
            console.log(err);
        }
